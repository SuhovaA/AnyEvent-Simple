# This Makefile is for the AnyEvent::Simple extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/AnyEvent/Simple.pm]
#     AUTHOR => [q[Anastasiya Sukhova <a.sukhova@local>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[perl]
#     NAME => q[AnyEvent::Simple]
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/AnyEvent/Simple.pm]
...
DIRFILESEP = /
DFSEP = $(DIRFILESEP)
NAME = AnyEvent::Simple
NAME_SYM = AnyEvent_Simple
VERSION = 0.01
...
MAKE = make
FULLEXT = AnyEvent/Simple
BASEEXT = Simple
PARENT_NAME = AnyEvent
DLBASE = $(BASEEXT)
VERSION_FROM = lib/AnyEvent/Simple.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
DISTNAME = AnyEvent-Simple
DISTVNAME = AnyEvent-Simple-0.01
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Anastasiya Sukhova <a.sukhova@local>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: AnyEvent-Simple' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "Anastasiya Sukhova <a.sukhova@local>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "AnyEvent-Simple",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="AnyEvent-Simple" VERSION="0.01">' > AnyEvent-Simple.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> AnyEvent-Simple.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Anastasiya Sukhova &lt;a.sukhova@local&gt;</AUTHOR>' >> AnyEvent-Simple.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> AnyEvent-Simple.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="darwin-thread-multi-2level-5.26" />' >> AnyEvent-Simple.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> AnyEvent-Simple.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> AnyEvent-Simple.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> AnyEvent-Simple.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'client.pl' '$(INST_LIB)/AnyEvent/client.pl' \
	  'lib/AnyEvent/Simple.pm' 'blib/lib/AnyEvent/Simple.pm' 
